...
def show_images(images, titles=None, ncols_per_row=5):
    ...
    if nrows < 1:
        nrows = 1
    figs, axes = plt.subplots(nrows=nrows, ncols=ncols_per_row, figsize=(15, 4 * nrows))
    if titles is None:
        titles = [""] * n
...
def get_cifar10_dataset(random_crop=False):
    ...
    if random_crop:
        train_transform = T.Compose(
            [T.RandomResizedCrop(32, scale=[0.5, 1.0]), T.ToTensor()]
        )
    else:
    ...
    datasets = {
        ...
        "test": D.CIFAR10("data", train=False, transform=transform),
    }
    return datasets
...
def make_dataloader(datasets: dict, batch_size: int):
    loader = {
        key: torch.utils.data.DataLoader(
            datasets[key], batch_size=batch_size, shuffle=key == "train"
        )
        for key in datasets
    }
    return loader
...
def simulate_scheduler(
    gamma, num_epochs, schd=torch.optim.lr_scheduler.ExponentialLR, **kwargs
):
    lrs = []
    dummy_model = nn.Linear(1, 1)
    dummy_optimizer = torch.optim.SGD(dummy_model.parameters(), 0.1)
    dummy_scheduler = schd(dummy_optimizer, gamma=gamma, **kwargs)
